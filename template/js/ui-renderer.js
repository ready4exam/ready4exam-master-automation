...
/* -----------------------------------
   MOTIVATIONAL FEEDBACK LOGIC (PRESERVED)
----------------------------------- */
        ...
        els.list.innerHTML = `
            <div class="space-y-6 text-left animate-fadeIn">
                ...
                <div class="bg-blue-50 p-6 rounded-2xl border-l-4 border-blue-600">
                    <span class="text-[10px] font-black uppercase tracking-widest text-blue-600 block mb-2">
                        Reason (R)
                    </span>
                    <div class="text-lg font-bold">${q.scenario_reason}</div>
                </div>
        ...
        els.list.innerHTML = `
            <div class="grid md:grid-cols-2 gap-8">
                ...
                <div class="bg-yellow-50 p-6 rounded-2xl">
                    <h3 class="font-black mb-2">💡 Study Hint</h3>
                    <p class="italic">${q.scenario_reason}</p>
                </div>
            </div>`;
        return;
    }
...
export function renderResults(stats, diff) {
    ...
    if (els.scoreBox) {
        ...
        els.scoreBox.innerHTML = `
            ...
            <div class="text-sm italic mt-2">${motivation}</div>
        `;
    }
}
    ...
    els.reviewContainer.innerHTML = qs.map((q, i) => {
        ...
        return `
            <div class="p-6 bg-white rounded-2xl border">
                <div class="font-black mb-3">Q${i + 1}. ${q.text}</div>
...
export function showStatus(msg, cls="") { initializeElements(); els.status.textContent = msg; els.status.className = cls; els.status.classList.remove("hidden"); }
