...
/* -----------------------------------
   BULLETPROOF AR NORMALIZER (RENDERER LEVEL)
----------------------------------- */
function normalizeAssertionReason(q) {
    ...
    const cleaned = cleanKatexMarkers(sources)
        .replace(/\s+/g, " ")
        .trim();

    // Extract Assertion
    let assertion = "";
    const aMatch = cleaned.match(/Assertion\s*\(A\)\s*:\s*(.*?)(?=Reason\s*\(R\)\s*:|$)/i);
    if (aMatch) assertion = aMatch[1].trim();

    // Extract Reason
    let reason = "";
    const rMatch = cleaned.match(/Reason\s*\(R\)\s*:\s*(.*)$/i);
...
export function renderQuestion(q, idx, selected, submitted) {
        ...
        els.list.innerHTML = `
            <div class="space-y-6">
                ...
                <div class="bg-blue-50 p-6 rounded-2xl border-l-4 border-blue-600">
                    ...
                    <div class="text-lg font-bold mt-2">${reason}</div>
                </div>
    ...
    /* ===== CASE STUDY (MOBILE-FIRST ORDER FIXED) ===== */
    if (type.includes("case")) {
        els.list.innerHTML = `
            <div class="space-y-6">
                ...
                <!-- Hint FIRST on mobile -->
                <div class="bg-yellow-50 p-6 rounded-2xl italic border border-yellow-200">
