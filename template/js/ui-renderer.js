...
export function renderQuestion(q, idx, selected, submitted) {
    ...
    // REVERTED TO ORIGINAL AR LAYOUT
...
export function renderResults(stats, diff) {
    ...
    if (els.scoreBox) {
        ...
        els.scoreBox.innerHTML = `
            ...
            <div class="text-sm md:text-lg text-gray-500 font-bold italic max-w-sm mx-auto px-4">${motivation}</div>
        `;
    }
    ...
    if (analysisBtn) {
        analysisBtn.onclick = () => {
            ...
            const categories = [
                { name: "The Recall Engine (Memory)", score: getScore('mcq'), high: "🏆 **Master Architect:** Your library of facts is huge!", low: "🌱 **Builder in Progress:** Reading the chapter like a story will help facts stick." },
                { name: "The Logic Linker (Reasoning)", score: getScore('ar'), high: "🧠 **Strategy Star:** You see how ideas connect!", low: "🔍 **Detective Trainee:** Ask 'Why?' to turn facts into logic." },
                { name: "The Problem Solver (Application)", score: getScore('case'), high: "🔬 **Real-World Hero:** You think like a great inventor!", low: "🚀 **Ready for Launch:** Every mistake here is just a test flight for your brain." }
            ];
            ...
            els.analysisContent.innerHTML = `
                <div class="space-y-5">
                    <div class="bg-gradient-to-r from-blue-600 to-indigo-600 p-5 rounded-3xl text-white shadow-lg">
                        ...
                        <p class="text-sm font-medium italic">"Hard questions make your brain stronger. You're getting better every minute!"</p>
                    </div>
...
export function renderAllQuestionsForReview(qs, ua) {
    ...
    els.reviewContainer.innerHTML = `
        <div class="mb-10 text-center px-4">
            ...
            <p class="text-slate-500 font-medium italic text-sm">Comparing ideas with facts is how experts are made!</p>
        </div>
        ${qs.map((q, i) => {
            ...
            <div class="p-6 bg-white rounded-[2rem] border-2 border-slate-50 shadow-sm mb-8 relative">
                ...
                <div class="flex gap-4 mb-6">
                    <span class="flex-shrink-0 w-10 h-10 rounded-2xl ${isCorrect ? 'bg-green-100 text-green-600' : 'bg-amber-100 text-amber-600'} flex items-center justify-center font-black text-lg">${i + 1}</span>
