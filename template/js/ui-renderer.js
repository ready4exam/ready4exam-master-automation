    ...
    if (type.includes("ar") || type.includes("assertion")) {
        ...
        els.list.innerHTML = `
            <div class="space-y-6 text-left animate-fadeIn">
                <div class="text-xl font-extrabold text-gray-900">Q${idx}. Assertion (A): ${assertion}</div>
                <div class="bg-blue-50 p-6 rounded-2xl border-l-4 border-blue-600 shadow-sm">
                    ...
                    <div class="text-lg font-bold text-gray-800">${reason}</div>
                </div>
...
export function renderResults(stats, diff) {
    ...
    if (analysisBtn) {
        analysisBtn.onclick = () => {
            ...
            const categories = [
                { name: "Memory (MCQ)", score: getScore('mcq'), high: "🏆 Master Architect!", low: "📖 Building foundations..." },
                { name: "Logic (A-R)", score: getScore('ar'), high: "🧠 Strategy Star!", low: "🔍 Pattern seeker..." },
                { name: "Application (Case)", score: getScore('case'), high: "🔬 Explorer Level!", low: "🌱 Apply theory more." }
            ];
            els.analysisContent.innerHTML = categories.map(cat => `
                <div class="p-4 rounded-2xl border ${cat.score >= 0.7 ? 'bg-green-50 border-green-100' : 'bg-indigo-50 border-indigo-100'}">
                    ...
                    <div class="w-full bg-gray-200 h-2 rounded-full overflow-hidden mb-2"><div class="h-full ${cat.score >= 0.7 ? 'bg-green-500' : 'bg-indigo-500'}" style="width: ${cat.score * 100}%"></div></div>
