...
//
// ✅ Phase-3 Final Build
// - Dynamically fetches from Supabase table (no schema cache issue)
// - Works with existing api.js (fetchQuestions({ table, difficulty }))
// - No regressions to auth, UI, or analytics
// -----------------------------------------------------------------------------
...
async function loadQuiz() {
  try {
    ...
    UI.showStatus(
      `<span class='text-red-600 font-semibold'>Failed to load quiz: ${err.message}</span>`
    );
  }
}
...
async function handleSubmit() {
  ...
  if (user) {
    ...
    try {
      ...
      console.warn("[ENGINE] Analytics logging failed:", e);
    }
  }
...
async function initQuizEngine() {
  ...
  // Hook up UI actions
  UI.bindNavigation(handleNavigation);
  UI.bindAnswerSelection(handleAnswerSelection);
  UI.bindSubmit(handleSubmit);
  UI.bindLogout(signOut);
