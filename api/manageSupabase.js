...
function normalizeDifficulty(d){
  if(!d)return"Simple";d=d.toLowerCase().trim();
...
function normalizeQType(t){
  if(!t)return"MCQ";t=t.toLowerCase().trim();
...
export default async function handler(req,res){
  ...
  try{
    ...
    const {meta,csv}=body||{};
    if(!meta||!csv||!Array.isArray(csv))return res.status(400).json({ok:false,error:"Missing meta/csv array."});
    ...
    if(!url||!key)return res.status(500).json({ok:false,error:"Supabase config missing."});
    ...
    const {data:ex}=await supabase.from("usage_logs").select("refresh_count").eq("table_name",table).single();

    if(ex){
      await supabase.from("usage_logs").update({
        refresh_count:ex.refresh_count+1,inserted_count:rows.length,updated_at:new Date(),
        class_name:meta.class_name,subject:meta.subject,book:meta.book,chapter:meta.chapter
      }).eq("table_name",table);
    }else{
      await supabase.from("usage_logs").insert({
        table_name:table,class_name:meta.class_name,subject:meta.subject,book:meta.book,chapter:meta.chapter,
        inserted_count:rows.length,refresh_count:0,created_at:new Date(),updated_at:new Date()
      });
    }

    return res.status(200).json({ok:true,message:"Table updated successfully.",new_table_id:table,inserted:data?.length||rows.length});
