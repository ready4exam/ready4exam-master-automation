...
export default async function handler(req, res) {
  ...
  try {
    ...
    /* ============================================================
       1. DETECT TABLE → CREATE OR REFRESH (AUTO — NO FRONTEND MODE NEEDED)
    ============================================================ */
    ...
    /* Always clear and re-insert new questions */
    ...
    /* ============================================================
       2. NEW LOGIC — FULL BACKEND ONLY
          No mode needed. Auto-detect via usage_logs table.
    ============================================================ */
    ...
    if (!ex) {
      ...
      console.log(`📌 FIRST RUN LOG CREATED → ${table}`);
    }

    /* ---------- REFRESH → INCREASE ONLY ---------- */
    else {
      await supabase.from("usage_logs")
        .update({
          refresh_count: (ex.refresh_count||0)+1,
          inserted_count: rows.length,
          updated_at: new Date()
        })
        .eq("id", ex.id);

      console.log(`🔄 REFRESH LOG UPDATED → ${table} +1`);
    }

    return res.status(200).json({
      ok:true,
      message:`${!ex?"Generated":"Refreshed"} Successfully`,
      new_table_id:table,
